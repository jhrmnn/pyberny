...
- `berny.solvers.XTBSolver`, a GFN2-xTB backend driving Grimme's `xtb` through its Python bindings (optional `xtb` extra: `pip install pyberny[xtb]`). It offers a smoother semiempirical potential-energy surface than PM7, which can be effectively discontinuous near flat minima.
