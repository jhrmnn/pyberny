...
- `berny.solvers.XTBSolver`, a GFN2-xTB backend driving Grimme's `xtb` through its Python bindings (install from conda-forge: `conda install -c conda-forge xtb-python`). It offers a smoother semiempirical potential-energy surface than PM7, which can be effectively discontinuous near flat minima.
