...
- `berny.solvers.XTBSolver`, a GFN-xTB backend (default GFN2-xTB) evaluated through the `tblite` library (optional `xtb` extra: `pip install pyberny[xtb]`). It offers a smoother semiempirical potential-energy surface than PM7, which can be effectively discontinuous near flat minima.
