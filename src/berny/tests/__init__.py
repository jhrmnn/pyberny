...
* :meth:`ModelPotential.start` -- a deliberately awkward starting geometry,
* :meth:`ModelPotential.energy` / :meth:`ModelPotential.gradient` -- the energy
  surface and its analytic gradient (energy in arbitrary but consistent units,
  length in angstrom, gradient per angstrom),
* :meth:`ModelPotential.assert_at_minimum` -- a check that a converged geometry
  sits at the known minimum, up to rigid motion and any soft (zero-curvature)
  modes.
