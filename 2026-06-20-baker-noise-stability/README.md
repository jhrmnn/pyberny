...
*Investigation, 2026-06-20. GFN2-xTB via `tblite`. Self-contained report; see
`scripts/` for the analysis code, `data/` for raw outputs, `structures/` for
geometries, and `minima_paths.png` for the figure.*

Each of the 30 Baker molecules was optimized from its reference start geometry
and from noisy copies produced by adding isotropic Gaussian noise to every
...
relative to the clean run. Raw data: `data/baker_noise_stability.json`.
...
- **The 14 hard failures are all at sigma >= 0.2 A** and come in three
  flavours (see the per-molecule table in `data/baker_noise_stability.md`):
...
This is the subject of a follow-up issue
([jhrmnn/pyberny#148](https://github.com/jhrmnn/pyberny/issues/148)); the
reference and lower-minimum geometries for these molecules are in
`structures/` (see `structures/manifest.md`).
...
resulting single piecewise-linear path.

![Interpolated energy paths through the Baker minima found under start-geometry noise](minima_paths.png)

The figure shows energy (log scale, kcal/mol from the lowest located minimum)
vs a cumulative-RMSD path coordinate, one panel per molecule, with every
minimum marked and labelled and the no-noise (reference) minimum highlighted by
a gold star. For methylamine, ethanol, and histidine the gold star sits *above*
another minimum -- the unperturbed run did not reach the lowest basin.
...
## Follow-up: is the benzene "second minimum" really a minimum?

The benzene path panel shows a +18.9 kcal/mol second "minimum" that looked
suspicious (originally raised as
[jhrmnn/pyberny#149](https://github.com/jhrmnn/pyberny/issues/149)). Two things
were established:

1. **That exact structure is unrecoverable.** `minima_paths.py` saved only
   energies, not geometries, and seeded its perturbations with Python's salted
   `hash()`, so it cannot be re-drawn.
2. **A 28-minute targeted hunt settled the underlying question.** Across **471
   benzene noisy optimizations, 461 converged** (0-120 kcal/mol): *none* landed
   in the +14...24 kcal/mol window (the +18.9 type never recurred; converged
   energies cluster at 0, +32.3, then a scatter up to ~120), and *none* was
   non-stationary — the maximum Cartesian gradient component over all 461 was
   6.2e-4 a.u. (just above the 4.5e-4 `gradientmax` threshold), with the
   internal-coordinate gradient that `Berny.is_converged` actually tests
   (`B_inv.T @ g`) at 4.4e-4 there. Internal ≈ Cartesian everywhere; no
   null-space gap.

So pyberny is **not** declaring convergence at non-stationary benzene
structures; the suspicious appearance was the normal threshold-level residual
plus a linear-interpolation artifact. Issue #149 was closed as not
reproducible. (The orthogonal question of whether the distorted *stationary*
structures are minima or saddles was not pursued.)
...
scripts/noise_stability.py --benchmark baker --seeds 6 \
    ...
    --out baker_noise_stability.md \
    --out-json baker_noise_stability.json

# Interpolated energy paths through the multiple minima (~25 min, 1 core)
scripts/minima_paths.py \
    --json baker_noise_stability.json \
    --out-fig minima_paths.png \
    --out-json minima_paths.json

# Reference / lower-minimum (+ benzene) structures in structures/ (~10 min)
scripts/extract_noise_minima.py
```

The scripts import `berny` from an installed pyberny (`pip install -e
".[benchmark]"` from a checkout of `master`). The high-sigma trials on the
larger flexible molecules dominate runtime; reduce `--seeds` / drop `0.3` for a
quick pass.
