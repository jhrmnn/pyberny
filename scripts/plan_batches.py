...
def emit(reference, solvers, nbins_arg, cache, cache_path=None):
    out = []
    ...
    for solver in solvers:
        ...
        per_call = measure(reference, solver, cache, cache_path)
        bins, totals, nbins = plan(reference, solver, nbins_arg, per_call)
...
def main(argv=None):
    ...
    print(emit(reference, args.solvers, args.nbins, cache, args.cache), end='')
    if args.cache:
